...
function createUser(data, id = null) {
    ...
    if (!data.chosenDateTime) {
        errorFactory.createInvalidDataError('Falta indicar el día y horario predilecto')
    } else {
        user.chosenDateTime = data.chosenDateTime;
    }

    if (!data.writingFrecuency) {
        errorFactory.createInvalidDataError('Falta indicar la frecuencia de escritura')
    } else {
        user.writingFrecuency = data.writingFrecuency
    }

    if (!data.writingGenre) {
        errorFactory.createInvalidDataError('Falta indicar su género de escritura predilecto')
    } else {
